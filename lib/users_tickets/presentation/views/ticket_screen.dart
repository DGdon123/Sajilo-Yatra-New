...
class _MyHelloState extends ConsumerState<TicketScreen> {
  String token = "";
  String veh = 'Bus';
  ...
  Future<void> loadData() async {
    ...
    setState(() {
      ...
      leaving = prefs.getString('leaving') ?? "Leaving From";
      going = prefs.getString('going') ?? "Going To";
      dob = prefs.getString('dob') ?? "Departure Date";
      vehicle2 = prefs.getString('type') ?? "Vehicle Type";
  ...
  appLogin() async {
    TicketModelRequest loginRequestModel = TicketModelRequest(
    ...
    if (registerSignKey.currentState!.validate()) {
      await ref.read(ticketControllerProvider.notifier).getTicketDetails(
...
// Remove data for the 'counter' key.
                await prefs.remove('leaving');
                await prefs.remove('going');
